...
struct bigInt{
	...
	void init(char str[]){
		...
		for(int i=len-1;i>-1;i--){
			if(str[i]<='9' && str[i] >= '0'){
				...
				digit[size++] = 10+(str[i]-'A');
			}
		}
	}
	friend ostream& operator <<(ostream &output,const bigInt &ans){
		for(int i=ans.size-1;i>-1;i--){
			if(ans.digit[i]<10) output<<ans.digit[i];
			else{
				char out = ans.digit[i]-10+'a';
				output<<out;
			}
		}
		return output;
	...
	bigInt operator / (const int a) const{
		...
		for(int i=size-1;i>-1;i--){
			int tmp = (remain*10+digit[i])/a;
			remain = (remain*10+digit[i])%a;
			ans.digit[ans.size++] = tmp;
		}
		...
		while(ans.digit[ans.size-1] == 0){
			ans.size--;
		}
		if(ans.size == 0) ans.init(0);
		return ans;
	}
...
int main(){
	...
	while(scanf("%d%d%s",&m,&n,&str)!=EOF){
		...
		while(!(tmp.size == 1 && tmp.digit[0]==0)){
			ans.digit[ans.size++] = tmp%n;
			tmp = tmp/n;
		}
		cout<<ans<<endl;
	}
